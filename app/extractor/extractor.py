...
from app.extractor.filter         import filter_extracted_skills
...
def extract_skills(raw_text: str, catalog: List[dict] = None) -> List[SkillEntry]:
    ...
    Stages:
      ...
      3. Merge & deduplicate
      4. LLM noise filter  ← NEW: removes non-sensical extractions via Groq
      5. Profile-classification mastery scoring (single Groq call + bloom decay)
      6. Cast to SkillEntry
    ...
    # ── Stage 4: LLM noise filter ─────────────────────────────────────────────
    # Removes company names, generic verbs, adjectives, boilerplate phrases,
    # and any other non-sensical entries that SkillNER/JobBERT over-extracted.
    # On API failure the full merged list is returned unchanged (safe fallback).
    filtered: List[dict] = []
    try:
        filtered = filter_extracted_skills(merged, raw_text, source="resume")
    except Exception as exc:
        logger.error("[Orchestrator] Stage 4 (filter) failed: %s", exc)
        filtered = merged   # Never block the pipeline on a filter failure

    if not filtered:
        logger.warning("[Orchestrator] Filter removed all skills — using merged list as fallback.")
        filtered = merged

    # ── Stage 5: Profile-classification mastery scoring ───────────────────────
    scored_skills: List[dict] = []
    try:
        scored_skills = compute_mastery_scores(filtered, raw_text, catalog or [])
    except Exception as exc:
    ...
    # ── Stage 6: Cast to strict SkillEntry TypedDict + assign confidence ─────
...
def extract_skills_from_jd(raw_text: str) -> List[SkillEntry]:
    ...
    JDs don't have mastery — they have requirements. This pipeline:
      ...
      3. Merge & deduplicate
      4. LLM noise filter  ← NEW: removes non-sensical extractions via Groq
      5. SKIP mastery scoring — assign mastery_score = 0.0 (neutral placeholder)
      6. Cast to SkillEntry
    ...
    # ── Stage 4: LLM noise filter ─────────────────────────────────────────────
    # The JD filter uses the original full raw_text as context (not jd_requirements)
    # because the model benefits from seeing the full role description to correctly
    # judge whether a borderline term is being used as a skill or a filler phrase.
    filtered: List[dict] = []
    try:
        filtered = filter_extracted_skills(merged, raw_text, source="jd")
    except Exception as exc:
        logger.error("[Orchestrator] Stage 4 (JD filter) failed: %s", exc)
        filtered = merged   # Never block the pipeline on a filter failure

    if not filtered:
        logger.warning("[Orchestrator] JD filter removed all skills — using merged list as fallback.")
        filtered = merged

    # ── Stage 5: SKIP mastery — JD skills get neutral 0.0 ────────────────────
    jd_skills = [
        {
            "taxonomy_id":      s["taxonomy_id"],
            "taxonomy_source":  s["taxonomy_source"],
